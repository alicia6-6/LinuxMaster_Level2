...
### 현재 데이터 현황 (2026-09-06 기준)

- 2020년 06월 ~ 2023년 12월, 총 12개 회차 · 960문제 (회차당 80문제)
- hwp 기출문제 원본(교사용: 문제·정답 표시 / 해설집: 문제·정답·커뮤니티 해설)을 파싱해 문제·보기·정답·이미지·해설을 추출했습니다.
- 해설은 원본 해설집(comcbt.com 커뮤니티 기고 해설)의 텍스트를 그대로 정리해 사용했으며, 해설이 아예 없던 10문제만 직접 작성해 보충했습니다.
- 파싱 과정에서 원본 hwp 한 문항(2023년 3차 26번)에 포함된 서식 오류(문장 중간에 끼어든 정답 기호)를 수동으로 교정했습니다.
- hwp 원본은 상업 CBT 사이트(comcbt.com)의 저작물이라 `.gitignore`로 제외하고 로컬(`cbt/`)에만 보관합니다.
